            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                  <span className="text-sm md:text-base font-black text-white uppercase tracking-tight">ALEX_RIDER</span>
          ...
          {orders.map((order) => (
            <div key={order.id} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl overflow-hidden">
              <div className="relative">
                ...
                <span className={`absolute top-2 md:top-3 right-2 md:right-3 text-[8px] font-bold text-white uppercase tracking-wider px-1.5 md:px-2 py-0.5 md:py-1 rounded-full ${order.statusColor}`}>
                  {order.status}
                </span>
              </div>
              <div className="p-3 md:p-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  <span className="text-[10px] md:text-xs font-black text-white uppercase tracking-tight leading-tight">{order.name}</span>
                  <span className="text-[10px] md:text-xs font-black text-[#ff2d78] shrink-0 ml-2">{order.price}</span>
                </div>
                <button className="w-full flex items-center justify-center gap-2 text-[9px] font-bold text-[#888] border border-[#2a2a2a] rounded-full py-2 md:py-2.5 uppercase tracking-widest hover:text-white hover:border-white transition-all">
                  <order.actionIcon size={12} className="md:hidden" strokeWidth={2} />
                  <order.actionIcon size={13} className="hidden md:block" strokeWidth={2} />
                  {order.action}
                </button>
              </div>
            </div>
